...
class InputUIUtil {
    ...
    handleOnInputEvent = (e, index) => {
        const otp_length        = this.vue_instance.props?.config?.otp_length || 6;
        const value             = e.target.value.replace(/\D/g, "").charAt(0) || "";
        const next_input_id     = `${this.vue_instance.props?.config?.id}_${index + 1}`;
        const next_input_el     = document.getElementById(next_input_id);


        this.vue_instance.props.config.value[index] = value;
    ...
    handleOnBackspaceEvent = (e, index) => {
        const otp_length        = this.vue_instance.props?.config?.otp_length || 6;
        const current_value     = this.vue_instance.props.config.value[index] || "";
        const next_input_id     = `${this.vue_instance.props?.config?.id}_${index - 1}`;
        const next_input_el     = document.getElementById(next_input_id);

        this.vue_instance.props.config.value[index] = "";
    ...
    handleOnPasteEvent = (e, index) => {
        ...
        [...pasted].forEach((char, i) => { this.vue_instance.props.config.value[i] = char; });

        const next_input_id     = `${this.vue_instance.props?.config?.id}_${pasted.length - 1}`;
        const next_input_el     = document.getElementById(next_input_id);
